    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (images, blind_mask) in enumerate(pbar):
            ...
            with autocast('cuda', enabled=use_amp):
                ...
                # Brightness preservation loss: keep denoised x̂₀
                # statistics close to original I_noisy (at low t only)
                low_t = t < 500
                if low_t.any():
                    with torch.no_grad():
                        ac = diff.alphas_cumprod[t]
                        sqrt_ac = diff.sqrt_alphas_cumprod[t]
                        sqrt_omac = diff.sqrt_one_minus_alphas_cumprod[t]
                    # x̂₀ = (x_t - √(1-ᾱ) * ε̂) / √ᾱ
                    sqrt_ac_b = sqrt_ac.view(-1,1,1,1)
                    sqrt_omac_b = sqrt_omac.view(-1,1,1,1)
                    x0_pred = (x_t - sqrt_omac_b * eps_pred) / sqrt_ac_b
                    # Match mean/std per image in batch
                    loss_bright = (
                        ((x0_pred.mean(dim=[1,2,3]) - images.mean(dim=[1,2,3]))**2).mean() +
                        ((x0_pred.std(dim=[1,2,3]) - images.std(dim=[1,2,3]))**2).mean()
                    ) * 0.1  # λ=0.1
