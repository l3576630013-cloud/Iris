...
def _match_histogram(source, reference):
    """Match histogram of source (denoised) to reference (original noisy).
    Preserves brightness/contrast while keeping denoised structure."""
    ...
    for b in range(source.shape[0]):
        for c in range(source.shape[1]):
            ...
            rank = torch.searchsorted(src_sorted, src).clamp(0, n - 1)
            matched[b, c] = ref_sorted[rank].reshape(source.shape[2:])
    return matched
...
def main():
    ...
    # Common options
    parser.add_argument('--preserve_brightness', action='store_true', default=True,
                        help='Match denoised histogram to original (preserves contrast)')
    ...
    if single_mode:
        ...
        # ---- Brightness preservation: match denoised histogram to original ----
        if args.preserve_brightness:
            denoised = _match_histogram(denoised, noisy_tensor)
